...
export class FicheroSolicitudComponent {
  ...
  descargar(): void {
    ...
    this.orquestadorService.getArchivoSolicitud(archivo, this.nombreCenad(), this.idSolicitud(), this.isCenad()!).subscribe({
      next: () => {
      },
      error: (err) => {
        console.error(err);
  ...
  recargarFichero() {
    this.orquestadorService.loadFicheroSolicitudSeleccionado(this.ficheroSignal()!.Id).subscribe({
